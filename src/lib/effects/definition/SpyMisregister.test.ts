    ...
    describe("perception modifier", () => {
        ...
        it("registers as a specific Townsfolk role when perceiveAs overrides roleId", () => {
            ...
            const spy = addEffectTo(
                makePlayer({ id: "s1", roleId: "spy" }),
                "spy_misregister",
                { perceiveAs: { roleId: "washerwoman", team: "townsfolk", alignment: "good" } }
            );
            const state = makeState({ players: [observer, spy] });
        ...
        it("applies overrides in all perception contexts", () => {
            ...
            // "alignment" context
            expect(
                perceive(spy, observer, "alignment", state).alignment
            ).toBe("good");
            // "team" context
            expect(perceive(spy, observer, "team", state).team).toBe(
                "townsfolk"
            );
        ...
        it("works regardless of observer role (not restricted)", () => {
            ...
            expect(
                perceive(spy, chef, "alignment", state).alignment
            ).toBe("good");
            expect(
                perceive(spy, empath, "alignment", state).alignment
            ).toBe("good");
            expect(
                perceive(spy, fortuneTeller, "alignment", state).alignment
            ).toBe("good");
        });
