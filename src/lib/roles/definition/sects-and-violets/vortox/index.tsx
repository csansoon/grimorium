...
registerRoleTranslations('vortox', 'en', en)
registerRoleTranslations('vortox', 'es', es)
...
const definition: RoleDefinition = {
  ...
  initialEffects: [{ type: 'vortox_rule', expiresAt: 'never' }],

  RoleReveal: DefaultRoleReveal,

  NightAction: ({ state, player, onComplete }) => {
    const { language } = useI18n()
    const roleT = getRoleTranslations('vortox', language)
        ...
        entries: [
          {
            ...
            message: [
              {
                type: 'text',
                content: `${player.name} targeted ${target.name} as the Vortox.`,
              },
            ],
            data: {
              roleId: 'vortox',
          ...
          <div className='px-4 py-6 text-center'>
            <div className='flex justify-center mb-4'>
              <div className='w-20 h-20 rounded-full flex items-center justify-center border bg-red-500/10 border-red-400/30'>
                <Icon name='star' size='3xl' className='text-red-300' />
              </div>
            </div>
