...
registerRoleTranslations('fang_gu', 'en', en)
registerRoleTranslations('fang_gu', 'es', es)
...
const definition: RoleDefinition = {
  ...
  nightOrder: 30,
  chaos: 82,
  distributionModifier: { outsider: 1, townsfolk: -1 },
  shouldWake: (game, player) =>
    isAlive(player) && (game.history.at(-1)?.stateAfter.round ?? 0) > 1,
  initialEffects: [{ type: 'fang_gu_jump', expiresAt: 'never' }],

  RoleReveal: DefaultRoleReveal,

  NightAction: ({ state, player, onComplete }) => {
    const { language } = useI18n()
    const roleT = getRoleTranslations('fang_gu', language)
        ...
        entries: [
          {
            ...
            message: [
              {
                type: 'text',
                content: `${player.name} targeted ${target.name} as the Fang Gu.`,
              },
            ],
            data: {
              roleId: 'fang_gu',
          ...
          <div className='px-4 py-6 text-center'>
            <div className='flex justify-center mb-4'>
              <div className='w-20 h-20 rounded-full flex items-center justify-center border bg-red-500/10 border-red-400/30'>
                <Icon name='ghost' size='3xl' className='text-red-300' />
              </div>
            </div>
